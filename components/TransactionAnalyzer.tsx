...
import { amount, classify, exportCsv, parseCsv, type Kind, type Transaction } from '@/lib/transactions';
...
export function TransactionAnalyzer() {
  ...
  function analyze(table = source, selected = columns) {
    ...
    try {
      ...
      if (new Set(columns).size !== 4) throw new Error('서로 다른 열을 선택하세요.');
      const next = table.slice(1).map((cells,i) => {
        try {
          if (cells.length !== table[0]?.length) throw new Error('열 개수가 제목 행과 다릅니다.');
          const rawDate = (cells[columns[0] ?? 0] || '').trim().split(/[ T]/)[0]?.replace(/[./]/g,'-') || '';
          const parts = /^(\d{4})-(\d{1,2})-(\d{1,2})$/.exec(rawDate);
          if (!parts) throw new Error('날짜는 YYYY-MM-DD 형식이어야 합니다.');
          const date = `${parts[1]}-${parts[2]!.padStart(2,'0')}-${parts[3]!.padStart(2,'0')}`;
          const d = new Date(date + 'T00:00:00Z');
          if (!Number.isFinite(d.getTime()) || d.toISOString().slice(0,10) !== date) throw new Error('존재하지 않는 날짜입니다.');
  ...
  return <div className="mt-8 space-y-6">
    <section className="rounded-xl border-2 border-brand bg-white p-5 space-y-4"><h2 className="text-xl font-bold">엑셀 파일만 넣으면 바로 분류</h2><p>XLSX·XLS·CSV, 최대 2MB·5,000행. 날짜·내용·입금·출금 열을 자동으로 찾습니다. 인식하지 못하는 형식만 직접 확인하세요. 여러 시트가 있으면 첫 번째 시트를 분석합니다.</p>
      ...
      <input aria-label="거래내역 엑셀 또는 CSV 파일" type="file" accept=".xlsx,.xls,.csv" onChange={async e=>{const file=e.target.files?.[0];e.target.value='';if(!file)return;setRows([]);setSource([]);if(file.size>2*1024*1024){setError('2MB 이하 파일을 선택하세요.');return;}try{const buffer=await file.arrayBuffer();if(/\.xlsx?$/i.test(file.name)){const XLSX=await import('xlsx');const book=XLSX.read(buffer,{type:'array',cellDates:true,dateNF:'yyyy-mm-dd',sheetRows:5032});const sheet=book.Sheets[book.SheetNames[0]||''];if(!sheet)throw new Error();load(XLSX.utils.sheet_to_csv(sheet,{dateNF:'yyyy-mm-dd'}));}else{load(new TextDecoder(encoding,{fatal:true}).decode(buffer));}}catch{setError('파일을 읽지 못했습니다. 암호를 해제한 엑셀 파일인지 확인하세요. CSV는 인코딩을 확인하세요.');}}}/>
      <div className="flex flex-wrap gap-3"><button className={button} onClick={()=>load('날짜,내용,입금,출금\n2026-09-01,급여,3000000,0\n2026-09-02,카페,0,5000\n2026-09-03,월세,0,500000\n2026-09-04,내 계좌 이체,0,200000')}>가상 예시로 체험</button><button className="rounded-lg border px-4 py-3" onClick={()=>{setSource([]);setRows([]);setError('');setMonth('전체');}}>전체 데이터 지우기</button></div>
    </section>
