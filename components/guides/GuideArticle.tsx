...
export function GuideArticle({ data }: { data: GuideData }) {
  ...
  return <Container className="py-10 sm:py-14">
    ...
    <article className="mx-auto mt-4 max-w-3xl">
      ...
      <div className="mt-10 space-y-10">{data.sections.map((section) => <section key={section.title}><h2 className="text-xl font-bold text-ink">{section.title}</h2>{section.paragraphs.map((paragraph) => <p key={paragraph} className="mt-3 text-sm leading-7 text-ink-soft">{paragraph}</p>)}</section>)}</div>
