  ...
  function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (kind === "weekly") {
        ...
        next = value.supported ? { lines: [{ label: "산정 거래금액", value: formatWon(value.transactionAmount ?? 0) }, { label: "적용 상한요율", value: `${((value.rate ?? 0) * 100).toFixed(1)}%` }, { label: "예상 중개보수 상한액", value: formatWon(value.maximumFee ?? 0), emphasis: true }], notice: "선택한 중개사무소 소재지의 주택 요율 기준이며 실제 보수는 상한 이내에서 협의합니다. 부가가치세는 자동 합산하지 않았습니다.", copy: `계산한눈에 중개보수 결과\n상한액: ${formatWon(value.maximumFee ?? 0)}\n부가세 별도` } : { lines: [{ label: "계산 상태", value: "지원 범위 밖", emphasis: true }], notice: value.reason ?? "관할 조례 확인이 필요합니다.", copy: `계산한눈에 중개보수 결과\n${value.reason}` };
      } else if (kind === "rent") {
...
function renderFields(kind: Exclude<Phase2CalculatorKind, "salary">, transaction: string, setTransaction: (value: string) => void, direction: string, setDirection: (value: string) => void) {
  if (kind === "weekly") return <><Field label="시급" name="hourlyWage" suffix="원" /><Field label="주당 소정근로시간" name="weeklyHours" suffix="시간" /><Field label="1주 소정근로일수" name="weeklyDays" suffix="일" defaultValue="5" /><SelectField label="개근 여부" name="attendance" options={OPTIONS.attendance} /><p className="text-sm text-ink-soft">통상근로자 주 40시간·5일 사업장 기준입니다. 주휴시간은 자동 산정하며 월 예상액에는 평균 4.345주를 적용합니다. 실제 월별 지급액은 달라질 수 있습니다.</p></>;
  if (kind === "savings") return <SavingsFields />;
  if (kind === "brokerage") return <><SelectField label="중개사무소 소재지" name="region" options={OPTIONS.region} /><SelectField label="부동산 유형" name="propertyType" options={OPTIONS.property} /><SelectField label="거래 유형" name="transaction" options={OPTIONS.transaction} onChange={(value) => setTransaction(value)} />{transaction === "monthlyRent" ? <><Field label="보증금" name="deposit" suffix="원" /><Field label="월차임" name="monthlyRent" suffix="원" /></> : <Field label="거래금액" name="amount" suffix="원" />}</>;
