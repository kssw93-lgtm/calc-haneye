...
export default function Page() {
  return <Container className="py-10 sm:py-14"><Breadcrumbs items={[{ label: "홈", href: "/" }, { label: "지원정책" }]} />
    ...
    {categories.map(category => <section key={category} className="mt-10"><div className="flex items-center gap-3"><PolicyCategoryIcon category={category} /><h2 className="text-xl font-bold text-ink">{category}</h2></div><div className="mt-4 grid gap-5 md:grid-cols-2">{entries.filter(([, item]) => item.category === category).map(([slug, item]) => <Link key={slug} href={"/policies/" + slug}><Card className="h-full"><div className="flex items-start gap-3"><PolicyCategoryIcon category={item.category} /><div><span className="text-xs font-semibold text-brand">{item.status}</span><h3 className="mt-1 text-lg font-bold text-ink">{item.title}</h3></div></div><p className="mt-3 text-sm leading-7 text-ink-soft">{item.summary}</p><p className="mt-3 text-xs text-ink-muted">확인일 {item.checkedAt}</p></Card></Link>)}</div></section>)}
  </Container>;
}
