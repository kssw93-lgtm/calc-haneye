...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <Container className="max-w-3xl py-10 sm:py-14">
    ...
    <article className="mt-5">
      ...
      {guide.sections.map((section, index) => <div key={section.title}><section className="mt-10 scroll-mt-24" id={`section-${index}`} key={section.title}><h2 className="text-xl font-bold">{section.title}</h2><p className="mt-3 leading-8 text-ink-soft">{section.body}</p></section>{index === Math.min(1, guide.sections.length - 1) && <CalculatorCTA hrefs={[calculator.href]} />}</div>)}
